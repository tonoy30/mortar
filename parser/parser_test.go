...
func testIntegerLiteral(t *testing.T, il ast.Expression, value int64) bool {
	interger, ok := il.(*ast.IntegerLiteral)
	if !ok {
		t.Errorf("il not *ast.IntegerLiteral. got=%T", il)
		return false
	}
	if interger.Value != value {
		t.Errorf("integ.Value not %d. got=%d", value, interger.Value)
		return false
	}
	if interger.TokenLiteral() != fmt.Sprintf("%d", value) {
		t.Errorf("integ.TokenLiteral not %d. got=%s", value, interger.TokenLiteral())
		return false
	}
	return true
}
